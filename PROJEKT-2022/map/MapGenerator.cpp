...
void MapGenerator::deleteShip(int w, int k, int board[10][10])
{
	for (int a = 0; a < 3; a++)
	{
		for (int b = 0; b < 3; b++)
...
void doGenerateHandMap(int board[10][10], bool start_game, void (*deleteShip)(int, int, int[10][10]))
{
	HANDLE hOut;
	...
	while (ifReady1 != "OK" && ifReady1 != "ok" && ifReady1 != "oK" && ifReady1 != "Ok" && ifReady1 != "POWROT" && ifReady1 != "Powrot" && ifReady1 != "powrot")
	{
		...
		if (ifReady1 != "OK" && ifReady1 != "ok" && ifReady1 != "oK" && ifReady1 != "Ok" && ifReady1 != "POWROT" && ifReady1 != "Powrot" && ifReady1 != "powrot")
		{
			...
			cout << "Podaj kolumne (A-J) oraz row (1-10): ";
			cin >> column;
			cin >> row1;
			if (row1 != 0 && column != 0 && ship1 != 1)
			{
				cout << "Podaj direction (1 - w gore, 2 - w prawo, 3 - w dol, 4 - w lewo): ";
				cin >> direction1;
				if (direction1 != 1 && direction1 != 2 && direction1 != 3 && direction1 != 4)
				{
					SetConsoleTextAttribute(hOut, FOREGROUND_RED);
					cout << "Podano zly direction.";
					Sleep(3000);
					continue;
				}
			}
