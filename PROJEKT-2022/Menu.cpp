...
void Menu::info(void(*frame)(string, bool, int), void(*setCursor)(int, int), void(*moving)(int&, bool&, int n))
{
    HANDLE hOut;
    ...
    while (end == false)
    {
    setCursor(40, 1);                                               // Informacje o autorze
...
int Menu::show(void(*frame)(string, bool, int), void(*setCursor)(int, int), void(*moving)(int&, bool&, int n))
{
    HANDLE hOut;
    hOut = GetStdHandle(STD_OUTPUT_HANDLE);
    bool end = false;
    ...
    while (end == false)
    {
        setCursor(1, 1);                                                //Wyœwietlenie menu g³ównego, poruszanie siê za pomoc¹ strza³ek, akcept enterem
            cout << endl << endl;
